...
#include <cstdint>
#include <string>
#define VERSION_MAJOR_NETLHGK 1
#define VERSION_MINOR_NETLHGK 3
/**
 * @brief 联恒光科通信协议 v1.3 协议打包类
 * 封装了协议帧构造函数，用于客户端/服务端构造完整 TCP 数据帧
 * 支持的帧类型包括：
 *  - 协议协商帧（0x08）
 *  - 控制命令帧（0x02）
 *  - 数据通道控制帧（0x07）
 ...
 *  - 外部数据推送频率帧（0x09）
 */
class NetLhgkProtocol
{
public:
	/**
	* @param version_major 主版本号，如 1
	* @param version_minor 子版本号，如 3
	*/
	NetLhgkProtocol(uint8_t version_major = 1, uint8_t version_minor = 3);
	 ...
	 * @param force double 格式的力值
	 * @param temp  double 格式的温度
	 */
...
private:
	...
	// 附加字段编码函数
	void appendBigEndian(std::vector<uint8_t>& buf, uint16_t val);
	void appendFloatBE(std::vector<uint8_t>& buf, float f);
	void appendDoubleBE(std::vector<uint8_t>& buf, double d);
	// CRC16/XMODEM 校验算法
