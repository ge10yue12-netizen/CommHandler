...
| P-RX-CTL | 助手出现控制事件记录（开始 / 停止等） |
| P-RX-ACK | 数据显示出现「协议应答」；摘要含自动应答 `code` 与线发 JSON（助手开库默认开启观测） |
| P-RX-UNP | 出现未解析接收（HEX 展示 + 运行日志）；兼容网口依赖 `bAssistObserve`（助手默认开） |
...
| 观测开关 | 助手开库后设 `bAssistObserve=true`（正式软件**不设**，DLL 默认关，控制态机与正式一致） |
...
**控制字约定（解析按 JSON 数值，键名任意）**

| 对端注入（等价） | 含义 | 线回语义（成功路径） |
|---|---|---|
| `{"tn":1}` 或 `{"cmd":1}` | 开始 | `code=0`，`tn=1`；助手「开始」+「协议应答」 |
| `{"tn":3}` 或 `{"cmd":3}` | 停止 | 已采集：`code=0`，`tn=3`，助手「停止」+「协议应答」；未采集：`code=170`，**无**「停止」事件 |

**正式软件对齐**：DLL 不擅自因开始置 `bOnlineCollect`；停止仅在采集中上报 `STOPCALC`。助手作为宿主在收到「开始/停止」后维护采集标志，故联调顺序须先开始再停止。
...
#### TC-NET-0-RX-01 控制开始（含自动应答）

| 项 | 内容 |
|---|---|
| 前置 | 协议 0；已连接；相机标志已开（助手默认 `bAllOpenCamera=true`） |
| 对端注入（文本） | `{"tn":1}`（与 `{"cmd":1}` 等价） |
| 对端注入 HEX | `7B 22 74 6E 22 3A 31 7D` |
| Fixture | `NET0_json_RX_ctrl_start.txt` |
| 对端应收到（语义） | `code=0`，`tn=1`，`message=null`；可含 `xlength`/`ylength` |
| Fixture（线回参考） | `NET0_json_TX_ack_start_ok.txt` |
| 判定 | P-RX-CTL（「开始」）；P-RX-ACK（`code=0`）；P-PEER-TXT |

#### TC-NET-0-RX-02 非法 JSON → 未解析

| 项 | 内容 |
|---|---|
| 前置 | 协议 0；已连接（助手已开 `bAssistObserve`） |
| 对端注入 HEX | `6E 6F 74 2D 6A 73 6F 6E`（`not-json`） |
| 判定 | P-RX-UNP |

#### TC-NET-0-RX-03 控制停止（须先成功开始）

| 项 | 内容 |
|---|---|
| 前置 | 已执行 TC-NET-0-RX-01 且助手出现「开始」 |
| 对端注入（文本） | `{"tn":3}`（与 `{"cmd":3}` 等价） |
| 对端注入 HEX | `7B 22 74 6E 22 3A 33 7D` |
| Fixture | `NET0_json_RX_ctrl_stop.txt` |
| 对端应收到（语义） | `{"code":0,"message":null,"tn":3}` |
| Fixture（线回参考） | `NET0_json_TX_ack_stop_ok.txt` |
| 判定 | P-RX-CTL（「停止」）；P-RX-ACK（`code=0`）；P-PEER-TXT |

#### TC-NET-0-RX-04 未开始直接停止 → 170、无停止事件

| 项 | 内容 |
|---|---|
| 前置 | 协议 0；已连接；**本会话未成功开始**（或已停止后再次停止） |
| 对端注入（文本） | `{"tn":3}` |
| 对端注入 HEX | `7B 22 74 6E 22 3A 33 7D` |
| Fixture | `NET0_json_RX_ctrl_stop.txt` |
| 对端应收到（语义） | `code=170`，`tn=3`，`message` 含 `Computation is not running` |
| Fixture（线回参考） | `NET0_json_TX_ack_stop_170.txt` |
| 判定 | P-RX-ACK（`code=170`）；**不得**出现新的「停止」控制事件；P-PEER-TXT |
...
| `NET0_json_RX_ctrl_stop.txt` | TC-NET-0-RX-03 / RX-04 |
| `NET0_json_TX_ack_start_ok.txt` | TC-NET-0-RX-01 线回参考 |
| `NET0_json_TX_ack_stop_ok.txt` | TC-NET-0-RX-03 线回参考 |
| `NET0_json_TX_ack_stop_170.txt` | TC-NET-0-RX-04 线回参考 |
...
| 2026-07-22 | §5.1 JSON：补停止/170 用例与协议应答判定；登记 `bAssistObserve`（助手开、正式软件关）；fixture 同步 |
