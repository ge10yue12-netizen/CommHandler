...
> 当前阶段：DLL 默认冻结；**仅允许按本版本登记的联恒索引重开**；原生通信助手优先  
...
联恒线格式（原 §3.3 BLOCKED）自 **V1.2.5** 起降为 `[VERIFIED]`（证据：`SerLhgkProtocol` / `NetLhgkProtocol` 源码，见 §14.4）；仍缺设备联调 Golden 时不得宣称「现场已验收」。
...
| 原则 | 基线决定 |
|---|---|
| DLL 冻结 | `[DECISION]` 默认不修改 CommHandler；**例外**：V1.2.5 起允许仅为重开联恒协议索引做最小改动（串口 `iProtocolType=5`、网口 `iProtoType=8`），不得借机重构其它协议 |
...
- 修改或重构现有 CommHandler DLL（**不含** V1.2.5 已批准的联恒索引重开）；
- Protocol Host 独立进程；
- Protobuf IPC；
- Lua 脚本系统；
- 通用协议可视化 IDE；
...
联恒（8）：数值发送须至少 2 个值（力、温）；依赖 DLL 流控/轮询标志（`bInquireSendFlag`）；错误帧类型网口为 `0x06`。
...
- 串口 5 联恒：数值发送至少 2 个（力、温）；错误帧类型为 `0x04`；不占用历史注释掉的 `case 0`，避免与三思冲突。
- `SendData(QString)` 为空实现 → 文本发送一律不支持。

### 14.4 联恒线格式事实（V1.2.5）

`[VERIFIED]` 证据源：`CommHandler/Protocol/Serial/SerLhgkProtocol.*`（串口 v1.1 小端）、`CommHandler/Protocol/Socket/NetLhgkProtocol.*`（网口 v1.3 大端）。

公共帧骨架：

- 帧头：`AA 55`
- 类型字节：`buf[2]`（控制 `0x02`、数据流控 `0x07`、模块数据 `0x01`、协商 `0x08`、轮询 `0x05`、速率 `0x09` 等）
- 帧尾：`0D 0A`；CRC16-XMODEM

差异：

| 项 | 串口 SerLhgk v1.1 | 网口 NetLhgk v1.3 |
|---|---|---|
| 长度/CRC 端序 | 小端 | 大端 |
| 错误帧类型 | `0x04` | `0x06` |
| 外发力/温类型号 | `0x0C` / `0x0D` | `0x0B` / `0x0C` |

索引约定（`[DECISION]`）：

- **不得**把联恒重新挂回串口/网口的 `0`（现分别为三思 / JSON）。
- 串口：`iProtocolType = 5`；网口：`iProtoType = 8`。

缺设备联调 Golden 时：助手与能力表可开放；现场验收结论不得标记 PASS。
...
联恒：线格式事实见 §14.4；无设备 Golden 时不得将现场联调标为里程碑通过。
...
| V1.2.5 | 2026-07-20 | DLL 冻结例外：重开联恒索引（串口 5 / 网口 8）；§14 能力表与 §14.4 线格式事实；解除联恒线格式 BLOCKED | 用户确认修改含解冻 DLL | SerLhgk/NetLhgk 源码 + CA-M2-UX-LHGK-001 |
...
- 本文档版本为 **V1.2.5**（含本节工具链）且本任务已填变更对齐单；已同时对齐 `docs/CODE_STYLE.md` 与 `docs/AUDIT_SPEC.md`；
...
V1.2.5 在 V1.2.4 基础上：登记联恒索引重开（串口 5 / 网口 8）、§14.4 线格式事实，并将 DLL 冻结改为「默认冻结 + 已批准例外」。变更流程仍强制对齐 `docs/AUDIT_SPEC.md`。

但：

- **满足 §21 之前，不得宣称“可以开始写代码 / 直接开工”。**
- 未验证资产与 Voltage/Pulse 阻塞风险仍然存在。
- 协议 Codec 与回放属于 M4。
- 联恒缺设备 Golden 时不得宣称现场验收通过。
